...
/// @dev BUILDERPACK-GENERATED-DATE: January 21, 2025
...
library BuilderPack {
    ...
    string constant GENDATE = "January 21, 2025";
...
library Chain {
    ...
    function Morpho_USDC_WETH(uint256 chainId) internal pure returns (KnownMorphoMarket memory) {
        if (chainId == 11155111) {
            ...
            revert NotFound("MorphoMarket for borrowing USDC against Wrapped Ether collateral not found on chain", chainId);
        }
    }
...
library ArbitrumMainnet {
    ...
    function knownAssets() internal pure returns (KnownAsset[] memory) {
        KnownAsset[] memory assets = new KnownAsset[](14);
        assets[0] = ETH();
        assets[1] = USDC();
        assets[2] = USDT();
        assets[3] = WBTC();
    ...
    function knownPriceFeeds() internal pure returns (KnownPriceFeed[] memory) {
        KnownPriceFeed[] memory priceFeeds = new KnownPriceFeed[](24);
        priceFeeds[0] = ETH_USD();
        priceFeeds[1] = USDC_USD();
        ...
        priceFeeds[10] = wUSDM_USD();
        priceFeeds[11] = ARB_USDT();
        priceFeeds[12] = WETH_USDT();
        priceFeeds[13] = wstETH_USDT();
        priceFeeds[14] = WBTC_USDT();
        priceFeeds[15] = GMX_USDT();
        priceFeeds[16] = weETH_WETH();
        priceFeeds[17] = rETH_WETH();
        priceFeeds[18] = wstETH_WETH();
        priceFeeds[19] = WBTC_WETH();
        priceFeeds[20] = rsETH_WETH();
        priceFeeds[21] = USDT_WETH();
        priceFeeds[22] = USDC_WETH();
        priceFeeds[23] = ezETH_WETH();
        return priceFeeds;
    }
...
library OptimismMainnet {
    ...
    function Comet_cWETHv3() internal pure returns (KnownComet memory) {
        ...
        collaterals[0] = KnownCometCollateral({
            asset:  0x1F32b1c2345538c0c6f582fCB022739c4A194Ebb,
            supplyCap: 1300000000000000000000,
            priceFeed: 0x295d9F81298Fc44B4F329CBef57E67266091Cc86,
            liquidationFactor: 9.7e17,
            borrowCollateralFactor: 8.8e17,
            liquidateCollateralFactor: 9.3e17
