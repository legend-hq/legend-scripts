...
contract QuarkBuilderBase {
    ...
    function constructOperationAndAction(
        ...
        if (Strings.stringEqIgnoreCase(actionType, Actions.ACTION_TYPE_TRANSFER)) {
            ...
            if (intent.amount == type(uint256).max) {
                intent.amount = getTotalAvailableBalance(
                    chainAccountsList,
                    payment,
                    bridgeFees,
            ...
            if (intent.amount == type(uint256).max) {
                ...
                uint256 totalAvailable = getTotalAvailableBalance(
                    chainAccountsList,
                    payment,
                    bridgeFees,
                    List.addUniqueUint256(chainIdsInvolved, intent.chainId),
    ...
    function getTotalAvailableBalance(
        ...
        uint256 paymentFees = Strings.stringEqIgnoreCase(payment.currency, assetSymbol) && !PaymentInfo.isOffchainPayment(payment)
