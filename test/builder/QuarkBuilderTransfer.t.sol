...
contract QuarkBuilderTransferTest is Test, QuarkBuilderTest {
    ...
    function testSimpleBridgeTransferWithQuotePaySucceeds() public {
        ...
        assertEq(
            ...
            "calldata is Multicall.run([cctpBridgeActionsAddress, quotePayAddress], [CCTPBridgeActions.bridgeUSDC(address(0xBd3fa81B58Ba92a82136038B25aDec7066af3155), 2e6, 6, bytes32(uint256(uint160(0xb0b))), usdc_(1), false), QuotePay.pay(Actions.QUOTE_PAY_RECIPIENT), USDC_1, 6e5, QUOTE_ID)]);"
        );
        assertEq(
            result.quarkOperations[0].expiry, BLOCK_TIMESTAMP + 7 days, "expiry is current blockTimestamp + 7 days"
        );
