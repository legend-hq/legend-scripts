...
contract QuarkBuilderCometBorrowTest is Test, QuarkBuilderTest {
        ...
        assertEq(
            ...
            abi.encode(
                Actions.MultiActionContext({
                    actionTypes: Arrays.stringArray(Actions.ACTION_TYPE_WRAP, Actions.ACTION_TYPE_BORROW),
    ...
    function testCometBorrowWithQuotePay() public {
        ...
        assertEq(result.quarkOperations[0].scriptSources.length, 0, "empty script source");
        assertEq(
            result.quarkOperations[0].expiry, BLOCK_TIMESTAMP + 7 days, "expiry is current blockTimestamp + 7 days"
        );
        ...
        assertEq(result.quarkOperations[1].scriptSources.length, 0, "empty script source");
        assertEq(
    ...
    function testBorrowPayFromBorrow() public {
        ...
        assertEq(
            ...
            abi.encode(
                Actions.MultiActionContext({
                    actionTypes: Arrays.stringArray(Actions.ACTION_TYPE_BORROW, Actions.ACTION_TYPE_QUOTE_PAY),
                    actionContexts: Arrays.bytesArray(
                        abi.encode(
                            Actions.BorrowActionContext({
                                amount: 2e6,
