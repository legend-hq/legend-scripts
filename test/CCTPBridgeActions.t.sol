...
contract CCTPBridge is Test {
    ...
    function testBridgeToBase() public {
        ...
        QuarkWallet.QuarkOperation memory op = new QuarkOperationHelper().newBasicOpWithCalldata(
            ...
            abi.encodeCall(
                ...
                (tokenMessenger, 500_000e6, 6, bytes32(uint256(uint160(address(wallet)))), USDC, false)
            ),
    ...
    function testBridgeMax() public {
        ...
        QuarkWallet.QuarkOperation memory op = new QuarkOperationHelper().newBasicOpWithCalldata(
            ...
            abi.encodeCall(
                CCTPBridgeActions.bridgeUSDC,
                (tokenMessenger, type(uint256).max, 6, bytes32(uint256(uint160(address(wallet)))), USDC, true)
            ),
            ScriptType.ScriptSource
        );
        bytes memory signature = new SignatureHelper().signOp(alicePrivateKey, wallet, op);
    ...
    function testBridgeMaxButCapped() public {
        vm.pauseGasMetering();
        QuarkWallet wallet = QuarkWallet(factory.create(alice, address(0)));
        bytes memory cctpBridgeScript = new YulHelper().getCode("BridgeScripts.sol/CCTPBridgeActions.json");
        ...
        QuarkWallet.QuarkOperation memory op = new QuarkOperationHelper().newBasicOpWithCalldata(
            ...
            abi.encodeCall(
                CCTPBridgeActions.bridgeUSDC,
                (tokenMessenger, 500_000e6, 6, bytes32(uint256(uint160(address(wallet)))), USDC, true)
            ),
            ScriptType.ScriptSource
        );
        bytes memory signature = new SignatureHelper().signOp(alicePrivateKey, wallet, op);
